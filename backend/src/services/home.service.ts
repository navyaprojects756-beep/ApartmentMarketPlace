...
export async function getHomeData(userId: string) {
  ...
  const [localSellers, outsideSellers, advertisements, homePromotions, globalCategories] = await Promise.all([
    ...
    prisma.globalCategory.findMany({ where: { isActive: true }, orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }] }),
  ]);

  const available = (sellers: typeof localSellers) => sellers.filter(seller => getSellerAvailability(seller).isOpen);
  return { mode, apartment: association, localSellers: available(localSellers), outsideSellers: available(outsideSellers), advertisements, homePromotions, globalCategories };
}
