...
const requestSchema = z.object({ title: z.string().trim().min(2).max(180), description: z.string().max(3000).optional(), type: z.enum(['PROMOTION', 'SELLER_ADVERTISEMENT', 'USER_ADVERTISEMENT', 'PRODUCT_PROMOTION', 'APARTMENT_ANNOUNCEMENT', 'EVENT', 'IMPORTANT_ALERT']), imageUrl: z.string().url().optional(), imageUrls: z.array(z.string().url()).max(10).optional(), sellerId: z.string().uuid().optional(), apartmentIds: z.array(z.string().uuid()).max(50).optional(), startAt: z.coerce.date().optional(), endAt: z.coerce.date().optional(), priority: z.number().int().min(0).max(100).default(0) });

advertisementRouter.post('/requests', requireAuth, async (request, response, next) => {
  try {
    const input = requestSchema.parse(request.body);
    const created = await prisma.advertisementRequest.create({ data: { requesterId: request.auth!.userId, sellerId: input.sellerId, title: input.title, description: input.description, type: input.type, imageUrl: input.imageUrl || input.imageUrls?.[0], imageUrls: input.imageUrls, startAt: input.startAt, endAt: input.endAt, priority: input.priority, targets: { create: (input.apartmentIds ?? []).map(apartmentId => ({ apartmentId })) } }, include: { targets: true } });
